...
export class ApproveGSTComponent implements OnInit {
    ...
    getIndex(){
      this.dtOptions = {
        ...
        "processing": true,
        'ajax' : {
            url : this.base_url+'getGstDataListAdmin',
            type : 'POST',
            data: {
    ...
    exportAsXLSX(){
      ...
      this.commonService.getApproveGSTData(sendData)
            .subscribe( response => {
              this.approveGstdata = response;
              //console.log(this.modelsdata);
             this.excelService.exportAsExcelFile(this.approveGstdata, 'ApproveGSTData');
            });
    }
