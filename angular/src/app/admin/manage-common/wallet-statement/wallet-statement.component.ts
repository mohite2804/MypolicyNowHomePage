...
@Component({
  ...
  styleUrls: ['./wallet-statement.component.css']
})
export class WalletStatementComponent implements OnInit {
  ...
  exportAsXLSX(){
    ...
    sendData.append('txn_type',this.formRecodEdit.value.filter_txn_type);
    sendData.append('txn_purpose',this.formRecodEdit.value.filter_txn_purpose);
    ...
    this.commonService.getWalletStatementData(sendData)
          .subscribe( response => {
  ...
  getWalletStatement(){
    const that = this;
      this.dtOptions = {
          ...
          "processing": true,
          'ajax' : {
              url : this.base_url+'admin/getWalletStatement',
              type : 'POST',
              data: {
