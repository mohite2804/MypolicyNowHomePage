...
export class MispsComponent implements  OnInit {
  ...
  getIndex(){
    ...
    const that = this;
      this.dtOptions = {
          ...
          "processing": true,
          'ajax' : {
              url : this.base_url+'admin/getmisplist',
              type : 'POST',
              data: {                
  ...
  exportAsXLSX(){
        ...
        this.commonService.getMispExcelExport(sendData)
        .subscribe( response => {
            this.mispData = response;
            if(this.mispData.status)
            {
              this.excelService.exportAsExcelFile(this.mispData.result, 'MISP_List');
