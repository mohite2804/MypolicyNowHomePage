...
export class CarInsuranceQuoteComponent implements OnInit {
  ...
  formValueChange(other_details,event){
    ...
    // console.log('policy_subtype_id:- ' +policy_subtype_id);
  ...
  checkSubTypeAndShowHideDiv(){
    ...
    if(foundComprehnsive){
      ...
      if(this.formQuoteDetails.value.policyholdertype==1){
        this.div_show_cpa_cover = false;
        this.validationForComprehnsive();
      }
    }
  ...
  ngOnInit(): void {
    ...
    if(this.loginUserType == 5){
      Swal.fire({
       ...
       // this.invoice_price_disabled_enabled = false;
       // this.validationFormSearchDetails();
        this.validationFormQuoteDetails();
  ...
  changeYearMonth(new_renew){
    ...
    if(new_renew == '1'){
      ...
      this.result_years_changes = this.result_years.slice( 0, 15);
    }
  ...
  setParameterForSubmitForm(){
    ...
    var  make_id :any = this.formQuoteDetails.value.make;
    make_id = parseInt(make_id);
    uploadData.append('make_id',make_id);


    var  model_id :any = this.formQuoteDetails.value.model;
    model_id = parseInt(model_id);
    uploadData.append('model_id',model_id);

    var variant_vehicle_str :any =  this.formQuoteDetails.value.variants;
    var variant_vehicle :any=  variant_vehicle_str.split("|",2);

    var  variant_id :any = variant_vehicle[1];
    variant_id = parseInt(variant_id);

    var  vehicle_id :any = variant_vehicle[0];
    vehicle_id = parseInt(vehicle_id);

    uploadData.append('variant_id',variant_id);
    uploadData.append('vehicle_id',vehicle_id);

    var  rto_id :any = this.formQuoteDetails.value.rto;
    ...
    var  manufacturer_month :any = this.formQuoteDetails.value.manufacturer_month_1;
    manufacturer_month = parseInt(manufacturer_month);
    uploadData.append('manufacturing_month',manufacturer_month);

    var  manufacturer_year :any = this.formQuoteDetails.value.manufacturer_year;
    ...
    var  od_discount :any = this.formQuoteDetails.value.od_discount;
    if(od_discount != "max"){ od_discount = parseInt(od_discount);  }
    uploadData.append('selected_od_discount',od_discount);

    var  proposer_type_id :any = this.formQuoteDetails.value.policyholdertype;
    proposer_type_id = parseInt(proposer_type_id);
    //alert(proposer_type_id);
    uploadData.append('proposer_type_id',proposer_type_id);

    var  carrier_type_id :any = this.formQuoteDetails.value.carrier_type_id;
    //if(carrier_type_id != ""){
      carrier_type_id = parseInt(carrier_type_id);
    //}
    uploadData.append('carrier_type_id',carrier_type_id);
    uploadData.append('vehicle_invoice_price',this.formQuoteDetails.value.invoice_price);

    var  previous_policy :any = this.formQuoteDetails.value.previous_policy;
    previous_policy = this.strToBool(previous_policy);
    uploadData.append('is_prev_policy_available',previous_policy);

    var  pre_previous_type :any = this.formQuoteDetails.value.pre_previous_type;
    pre_previous_type = parseInt(pre_previous_type);
    uploadData.append('prev_policy_type_id',pre_previous_type);

    var prev_policy_expiry_date :any = this.formQuoteDetails.value.pre_policy_expire_date;
    if(this.policy_sub_type_id == 4 || this.policy_sub_type_id == 20){
      var prev_policy_expiry_date  = this.formQuoteDetails.value.pre_policy_expire_date_for_od;
    }
    ...
    var  cpa_cover_reason :any = this.formQuoteDetails.value.cpa_cover_reason;
    ...
    var current_ncb_per : any = (this.formQuoteDetails.value.previous_ncb == "" || this.formQuoteDetails.value.previous_ncb == null) ? 0 : this.formQuoteDetails.value.previous_ncb;
